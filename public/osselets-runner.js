(() => {
  ...
  const OSSELET_IMG_DIR = "osselets/";
  const AUDIO = {
  ...
  const HERO_SCALE_X = 1.7;
  const HERO_SCALE_Y = 1.5;
  const HERO_FOOT_ADJ_PX = 12;
  const BEAR_SCALE = 1.5;
  const GROUND_Y = 440;
  const WORLD_LEN_L1 = 4200;
  const WORLD_LEN_L2 = 4600;
  ...
  async function loadImageSmart(file) {
    ...
    logOnce("img_" + file, "[osselets] image introuvable: " + file + " (essay\xE9: " + IMG_BASES.map((b) => b + file).join(", ") + ")");
    return null;
  }
  ...
  function AstragalusRunner() {
    ...
    useEffect(() => {
      let canceled = false;
      (async () => {
        const j = await fetchJSON("hero.anim.json");
        if (canceled)
          return;
        if (!j) {
          heroAnim.current = null;
          logOnce("herojson", "[osselets] hero.anim.json introuvable");
          return;
        }
    ...
    useEffect(() => {
      (async () => {
        const arr = [];
        for (let i = 1; i <= 5; i++) {
          const im = await loadImageSmart(OSSELET_IMG_DIR + `osselet (${i}).png`);
          if (im)
            arr.push(im);
        }
    ...
    const eyes = useRef([]);
    const miasmas = useRef([]);
    const intermittents = useRef([]);
    ...
    function pushEdu(msg, ms = 5200) {
      ...
      if (eduLog.current.length > 8)
        eduLog.current.shift();
    }
    const intro = useRef({ step: 0, t: 0 });
    useEffect(() => {
      function down(e) {
        if (["ArrowLeft", "ArrowRight", " ", "Space", "m", "M", "p", "P", "s", "S", "c", "C", "w", "W", "o", "O", "Enter"].includes(e.key))
          e.preventDefault();
        if (inIntro) {
          if (e.key === "ArrowRight") {
            intro.current.step = Math.min(5, intro.current.step + 1);
            intro.current.t = 0;
          }
          if (e.key === "Enter") {
            setInIntro(false);
      ...
      function up(e) {
        if (inIntro || learnModal || summaryOpen)
          return;
        if (e.key === "ArrowLeft")
    ...
    function press(name, v) {
      onScreenKeys.current[name] = v;
    }
    function clearTouchKeys() {
      onScreenKeys.current = { left: false, right: false, jump: false };
    }
    function softResetForLevel() {
      Object.assign(player.current, { x: 120, y: GROUND_Y - 68, vx: 0, vy: 0, onGround: true, facing: 1, speedMul: 1, dirt: 0, runPhase: 0, coyote: 0, jumpBuf: 0 });
      eyes.current = [];
      miasmas.current = [];
      intermittents.current = [];
      wardTimer.current = 0;
      oracleTimer.current = 0;
      active.current = { name: null, until: 0 };
      bear.current = { x: -999, y: GROUND_Y - 60, w: 64, h: 60, vx: 0, active: false, until: 0 };
      stage.current = "intro";
      setMessage(level === 1 ? "\u2190 \u2192 bouger \u2022 Espace sauter \u2022 S/C/W pour activer les amulettes apprises" : "\u2190 \u2192 bouger \u2022 Espace sauter \u2022 O (Oracle) pour r\xE9v\xE9ler les pi\xE8ges intermittents");
      setLearnModal(null);
      setEdu(null);
    ...
    function tryActivate(name) {
      if (!inv.current[name]) {
        pushEdu("Tu n\u2019as pas encore cette amulette \u2014 avance pour la trouver.");
        return;
      }
      const now = performance.now();
      const DUR = { speed: 8, purify: 0.4, ward: 8, oracle: 6 };
      active.current = { name, until: now + (DUR[name] || 0) * 1e3 };
      if (name === "speed")
        player.current.speedMul = 1.6;
      if (name === "purify")
        player.current.dirt = Math.max(0, player.current.dirt - 0.6);
      if (name === "ward")
    ...
    function setupPickups() {
      ...
      miasmas.current.length = 0;
      intermittents.current.length = 0;
      if (level === 1) {
        pickups.current.push({ x: 900, type: "speed", label: "Vitesse", pngKey: "speed" });
        miasmas.current.push({ x: 1950, w: 140 });
        pickups.current.push({ x: 2e3, type: "purify", label: "Purif.", pngKey: "purify" });
    ...
    useEffect(() => {
      const ctx = ctxRef.current || (canvasRef.current ? canvasRef.current.getContext("2d") : null);
      if (!ctx)
        return;
      ...
      const tick = (t) => {
        ...
        if (dtms > 66)
          dtms = 66;
        last = t;
        const slow = oracleTimer.current > 0 ? 0.88 : 1;
        const dt = dtms / 16.666 * slow;
        if (!paused && !modalBlocks)
          update(dt, dtms / 1e3);
        render(ctx);
    ...
    function update(dt, dtSecs) {
      ...
      if (active.current.name && performance.now() > active.current.until) {
        if (active.current.name === "speed")
          p.speedMul = 1.2;
        active.current.name = null;
      }
      if (wardTimer.current > 0)
        wardTimer.current = Math.max(0, wardTimer.current - dtSecs);
      if (oracleTimer.current > 0)
        oracleTimer.current = Math.max(0, oracleTimer.current - dtSecs);
      ...
      if (heroState.current.name !== nextName)
        heroState.current = { name: nextName, t: 0 };
      else
        heroState.current.t += dt * ANIM_SPEED;
      for (const pk of pickups.current) {
        ...
        if (Math.abs(p.x - pk.x) < 18 && Math.abs(p.y + p.h - GROUND_Y) < 26) {
          ...
          if (pk.type === "speed") {
            ...
            setLearnModal({ title: "NIK\xCA \u2014 Vitesse", key: "S", body: "Le talus (astragale) = os du mouvement. Active-la avec S pour distancer un danger." });
          }
          if (pk.type === "purify") {
            inv.current.purify = true;
            setLearnModal({ title: "KATHARSIS \u2014 Purification", key: "C", body: "Polissage/rituel : l\u2019os devient \u2018propre\u2019. Active C pour dissiper la salissure (maladie/miasme)." });
          }
          if (pk.type === "ward") {
            inv.current.ward = true;
            setLearnModal({ title: "APOTROPA\xCFON \u2014 Bouclier", key: "W", body: "Amulette contre le \u2018mauvais \u0153il\u2019. Active W pour repousser les yeux malveillants." });
          }
          if (pk.type === "oracle") {
            inv.current.oracle = true;
            setLearnModal({ title: "MANTIS \u2014 Oracle", key: "O", body: "Divination par osselets : r\xE9v\xE8le les pi\xE8ges intermittents. Active O pour les voir \xE0 l\u2019avance." });
          }
        }
      }
      for (const mz of miasmas.current) {
        if (p.x + p.w > mz.x && p.x < mz.x + mz.w) {
          p.speedMul = Math.max(0.8, p.speedMul - 8e-3);
          player.current.dirt = Math.min(0.9, player.current.dirt + 4e-3);
        }
      }
      if (level === 1) {
        if (!bear.current.active && p.x > 3400) {
          ...
          bear.current.vx = 2.8;
          bear.current.until = performance.now() + 7e3;
        ...
        if (bear.current.active) {
          ...
          if (bearAnim.current)
            bearAnim.current.t += dt * ANIM_SPEED;
          if (bear.current.x + bear.current.w > p.x) {
            p.x += 8;
            p.vx = Math.max(p.vx, 1);
            player.current.dirt = Math.min(1, player.current.dirt + 0.05);
            playOne(sfxOuchEl);
          }
          if (performance.now() > bear.current.until) {
            bear.current.active = false;
            setMessage("Il repart au loin.");
          }
        }
        if (stage.current !== "eyesWave" && p.x > 3200) {
          ...
          setMessage("Vague du \u2018mauvais \u0153il\u2019 ! Active W pour t\u2019en prot\xE9ger.");
        }
      }
      if (level === 2) {
        ...
        for (const ob of intermittents.current) {
          ...
          if (visible) {
            if (rectOverlap(ob.x, ob.y, ob.w, ob.h, p.x, p.y, p.w, p.h)) {
              p.x -= 12;
              p.vx = 0;
              player.current.dirt = Math.min(1, player.current.dirt + 0.04);
              playOne(sfxOuchEl);
            }
          }
        }
        if (stage.current !== "eyesLoop" && p.x > 1800) {
          stage.current = "eyesLoop";
          setMessage("Obstacles intermittents \u2014 O pour pr\xE9voir, W contre les yeux, C pour nettoyer.");
        }
      ...
      for (const e of eyes.current) {
        ...
        if (rectOverlap(e.x - 10, e.y - 6, 20, 12, p.x, p.y, p.w, p.h)) {
          if (wardTimer.current > 0) {
            ...
            setTimeout(() => {
              p.speedMul = Math.min(1.2, p.speedMul + 0.2);
            }, 1200);
            e.alive = false;
            playOne(sfxOuchEl);
          }
    ...
    function render(ctx) {
      ...
      for (let x = 300; x < WORLD_LEN; x += 420)
        drawColumn(ctx, x, GROUND_Y);
      ctx.fillStyle = "rgba(16,185,129,.18)";
      for (const mz of miasmas.current) {
        ctx.fillRect(mz.x, GROUND_Y - 12, mz.w, 12);
      }
      if (level === 2) {
        for (const ob of intermittents.current) {
          const visible = Math.sin(ob.t * 1.2) > 0;
      ...
      if (bear.current.active)
        drawBear(ctx, bear.current.x, bear.current.y);
      for (const e of eyes.current) {
        if (e.alive)
          drawEvilEye(ctx, e.x, e.y, oracleTimer.current > 0);
      }
    ...
    function renderIntro(ctx) {
      ...
      ctx.lineWidth = 2;
      ctx.fillRect(pad, panelY, WORLD_W - 2 * pad, panelH);
      ctx.strokeRect(pad, panelY, WORLD_W - 2 * pad, panelH);
      ctx.restore();
      ctx.fillStyle = "#0f172a";
    ...
    function centerText(ctx, txt, y, size = 22) {
    ...
    function drawColumn(ctx, baseX, groundY) {
      ...
      ctx.fillRect(-18, groundY - 140, 36, 10);
      ctx.fillRect(-18, groundY - 10, 36, 10);
      ctx.restore();
    }
    function drawAmulet(ctx, x, y, label, png, type) {
      ...
      ctx.beginPath();
      ctx.moveTo(-18, -12);
      ctx.quadraticCurveTo(0, -24 - 6 * Math.sin(performance.now() * 2e-3), 18, -12);
      ctx.stroke();
      if (png) {
        ctx.imageSmoothingEnabled = false;
        ctx.drawImage(png, -32, -32, 64, 64);
      } else {
        ...
        ctx.textAlign = "center";
        ctx.fillText(type === "oracle" ? "O" : "", 0, 4);
      }
      ctx.fillStyle = "#0f172a";
    ...
    function drawBear(ctx, x, y) {
      ...
      const W0 = 64, H0 = 60;
      const Wd = Math.round(W0 * BEAR_SCALE), Hd = Math.round(H0 * BEAR_SCALE);
      ctx.save();
      ctx.translate(x, y + H0);
      if (ba && ba.frames && ba.frames.length) {
        ...
        ctx.fillRect(0, -Hd + 20 * BEAR_SCALE, Wd, Hd - 24 * BEAR_SCALE);
        ctx.beginPath();
        ctx.arc(Wd - 10 * BEAR_SCALE, -Hd + 26 * BEAR_SCALE, 14 * BEAR_SCALE, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "#fde68a";
        ctx.fillRect(Wd - 6 * BEAR_SCALE, -Hd + 22 * BEAR_SCALE, 3 * BEAR_SCALE, 3 * BEAR_SCALE);
      }
      ctx.restore();
    }
    ...
    function drawHero(ctx, x, y, w, h, facing, dirt, runPhase, wardLeft) {
      ...
      if (wardLeft > 0) {
        const pct = Math.min(1, wardLeft / 8);
        const rad = 44 * HERO_SCALE_Y + 6 * Math.sin(performance.now() * 6e-3);
      ...
      const clip = anim && anim.clips ? anim.clips[heroState.current.name] : null;
      if (clip && clip.frames && clip.frames.length) {
        ...
        ctx.fillRect(10 + legA * 0.2, dh - 16, 8, 16);
        ctx.fillRect(dw - 18 + legB * 0.2, dh - 16, 8, 16);
        ctx.fillStyle = "#92400e";
        ctx.fillRect(10 + legA * 0.2, dh - 2, 10, 2);
        ctx.fillRect(dw - 18 + legB * 0.2, dh - 2, 10, 2);
        ctx.fillStyle = "#334155";
        ctx.fillRect(8, 28, dw - 16, 14);
        ctx.fillStyle = "#e5e7eb";
        ctx.beginPath();
        ctx.moveTo(12, 20);
        ctx.lineTo(dw - 12, 20);
        ...
        for (let i = 0; i < 3; i++) {
          ctx.beginPath();
          ctx.moveTo(16 + i * 8, 22);
          ctx.lineTo(20 + i * 8, 46);
          ctx.stroke();
        }
        ...
        ctx.stroke();
        ctx.beginPath();
        ctx.moveTo(dw / 2 - 8, 22);
        ctx.quadraticCurveTo(dw / 2, 18, dw / 2 + 8, 22);
        ctx.stroke();
        ctx.fillStyle = "#f8fafc";
        ctx.beginPath();
        ctx.arc(dw / 2, 12, 8, 0, Math.PI * 2);
        ctx.fill();
      }
      if (dirt > 0.01) {
        ctx.globalAlpha = Math.max(0, Math.min(dirt, 0.8));
    ...
    function drawOliveTrees(ctx, off) {
      ctx.save();
      ctx.translate(-off, 0);
      for (let x = 0; x < WORLD_W + WORLD_LEN; x += 260) {
    ...
    function drawFrieze(ctx, off) {
      ctx.save();
      ctx.translate(-off, 0);
      for (let x = 0; x < WORLD_W + WORLD_LEN; x += 180) {
    ...
    function drawHUD(ctx) {
      ...
      ctx.fillRect(12, 56, barW, 62);
      ctx.strokeRect(12, 56, barW, 62);
      const slots = level === 1 ? [
      ...
      let info = "";
      if (wardTimer.current > 0)
        info = `Bouclier ${wardTimer.current.toFixed(1)}s`;
      if (oracleTimer.current > 0)
        info = `Oracle ${oracleTimer.current.toFixed(1)}s`;
      if (info) {
        ctx.fillText(info, 12 + barW + 12, 84);
      }
      ctx.fillStyle = musicOn ? "#16a34a" : "#ef4444";
      ctx.beginPath();
      ctx.arc(12 + barW + 12, 110, 6, 0, Math.PI * 2);
      ctx.fill();
      ctx.fillStyle = "#0f172a";
      ctx.fillText("Musique (M)", 12 + barW + 26, 114);
      ctx.restore();
      if (summaryOpen) {
    ...
    function drawAmuletMini(ctx, cx, cy, png, type, owned) {
      ...
      if (png) {
        ...
        ctx.fillStyle = "#111827";
        ctx.textAlign = "center";
        ctx.fillText(type === "oracle" ? "O" : "", 0, 3);
      }
      ctx.restore();
    }
    function drawEduToast(ctx, text) {
      ...
      ctx.strokeStyle = "#cbd5e1";
      ctx.lineWidth = 2;
      ctx.fillRect(x, y, boxW, 82);
      ctx.strokeRect(x, y, boxW, 82);
      ctx.fillStyle = "#0f172a";
      ctx.font = "14px ui-sans-serif, system-ui";
      wrapText(ctx, text, x + 12, y + 26, boxW - 24, 18);
      ctx.restore();
    }
    ...
    const startBtnStyle = {
      ...
      color: "#fff",
      cursor: "pointer",
      boxShadow: "0 6px 14px rgba(5,150,105,.25)",
      position: "absolute",
      bottom: 16,
      left: "50%",
    ...
    const btn = (disabled = false) => ({ padding: "8px 12px", border: "1px solid #e5e7eb", borderRadius: 12, background: "#fff", cursor: disabled ? "default" : "pointer", opacity: disabled ? 0.5 : 1 });
    const btnDark = () => ({ padding: "8px 12px", border: "1px solid #111827", borderRadius: 12, background: "#111827", color: "#fff", cursor: "pointer" });
    const primaryBtn = (disabled = false) => ({ padding: "10px 14px", border: "1px solid #059669", borderRadius: 14, background: "#059669", color: "#fff", cursor: disabled ? "default" : "pointer", opacity: disabled ? 0.5 : 1, boxShadow: "0 6px 14px rgba(5,150,105,.25)" });
    return /* @__PURE__ */ React.createElement("div", {
      className: "min-h-screen w-full",
      style: { background: "linear-gradient(135deg,#fafaf9,#e7e5e4)", color: "#111827" }
    ...
    }, "Runner 2D \u2013 Amulettes d\u2019astragale"), /* @__PURE__ */ React.createElement("div", {
      style: { display: "flex", gap: 8, flexWrap: "wrap" }
    }, /* @__PURE__ */ React.createElement("button", {
      onClick: toggleMusic,
      style: btn()
    }, " ", musicOn ? "Musique ON" : "Musique OFF", " "), /* @__PURE__ */ React.createElement("button", {
      onClick: () => setPaused((v) => !v),
      disabled: inIntro || summaryOpen || !!learnModal,
      style: primaryBtn(inIntro || summaryOpen || !!learnModal)
    }, paused ? "Lecture" : "Pause"), /* @__PURE__ */ React.createElement("label", {
      style: btn()
      ...
      style: { marginLeft: 6 }
    }, "Mode mobile")), mobileMode && /* @__PURE__ */ React.createElement("label", {
      style: btn()
    }, /* @__PURE__ */ React.createElement("input", {
      type: "checkbox",
      checked: oneButton,
      onChange: (e) => setOneButton(e.target.checked)
    }), /* @__PURE__ */ React.createElement("span", {
      style: { marginLeft: 6 }
    }, "1 bouton")))), /* @__PURE__ */ React.createElement("p", {
      className: "text-sm",
      style: { color: "#475569", marginBottom: 12 }
    }, inIntro ? level === 1 ? "Niveau 1 : d\xE9couvre Vitesse (S), Purification (C) et Bouclier (W)." : "Niveau 2 : d\xE9couvre Oracle (O) et anticipe les obstacles intermittents." : "\u2190 \u2192 bouger \u2022 Espace sauter \u2022 S/C/W (LVL1) \u2022 O (LVL2) \u2022 P pause \u2022 M musique"), /* @__PURE__ */ React.createElement("div", {
      className: "bg-white",
      style: { border: "1px solid #e5e7eb", borderRadius: 14, padding: 12, boxShadow: "0 6px 16px rgba(0,0,0,.05)" }
    }, /* @__PURE__ */ React.createElement("div", {
      ref: wrapperRef,
      className: "w-full",
      style: { position: "relative", overflow: "hidden", border: "1px solid #e5e7eb", borderRadius: 12 }
    }, /* @__PURE__ */ React.createElement("canvas", {
      ref: canvasRef
    }), inIntro && /* @__PURE__ */ React.createElement("button", {
      onClick: startGame,
      style: startBtnStyle
    }, "Start"), learnModal && /* @__PURE__ */ React.createElement("div", {
      style: { position: "absolute", inset: 0, background: "rgba(255,255,255,.96)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, zIndex: 6 }
    }, /* @__PURE__ */ React.createElement("div", {
      style: { width: "min(720px,100%)", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 16, padding: 16, boxShadow: "0 8px 24px rgba(0,0,0,.08)" }
    }, /* @__PURE__ */ React.createElement("h3", {
      style: { fontWeight: 700, fontSize: 18, marginBottom: 8 }
    }, learnModal.title), /* @__PURE__ */ React.createElement("p", {
      style: { margin: "8px 0 12px", color: "#334155" }
    }, learnModal.body), /* @__PURE__ */ React.createElement("div", {
      style: { display: "flex", gap: 8, justifyContent: "flex-end" }
    }, /* @__PURE__ */ React.createElement("button", {
      onClick: () => setLearnModal(null),
      style: primaryBtn(false)
    }, "OK, reprendre (touche ", learnModal.key, ")")))), summaryOpen && /* @__PURE__ */ React.createElement("div", {
      style: { position: "absolute", inset: 0, background: "rgba(255,255,255,.95)", backdropFilter: "blur(4px)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }
    }, /* @__PURE__ */ React.createElement("div", {
      style: { width: "min(860px,100%)", background: "#fff", border: "1px solid #e5e7eb", borderRadius: 16, padding: 16, boxShadow: "0 8px 24px rgba(0,0,0,.08)" }
    }, /* @__PURE__ */ React.createElement("div", {
      style: { display: "flex", justifyContent: "space-between", alignItems: "center" }
    }, /* @__PURE__ */ React.createElement("h2", {
      style: { fontWeight: 600 }
    }, "Fin du niveau ", level, " \u2705"), /* @__PURE__ */ React.createElement("span", {
      style: { fontSize: 12, padding: "2px 8px", borderRadius: 999, background: "#111827", color: "#fff" }
    }, "Synth\xE8se")), /* @__PURE__ */ React.createElement("div", {
      style: { display: "grid", gap: 12, gridTemplateColumns: "1fr 1fr", marginTop: 12 }
    }, /* @__PURE__ */ React.createElement("div", {
      style: { background: "#fafafa", border: "1px solid #e5e7eb", borderRadius: 12, padding: 12, fontSize: 14 }
    }, /* @__PURE__ */ React.createElement("h3", {
      style: { fontWeight: 600, marginBottom: 8 }
    }, "Ce que tu as vu"), /* @__PURE__ */ React.createElement("ul", {
      style: { paddingLeft: 18, margin: 0 }
    }, level === 1 ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("li", null, /* @__PURE__ */ React.createElement("strong", null, "Vitesse"), " (NIK\xCA) : acc\xE9l\xE9rer pour \xE9chapper au danger (ours)."), /* @__PURE__ */ React.createElement("li", null, /* @__PURE__ */ React.createElement("strong", null, "Purification"), " (KATHARSIS) : retirer miasmes/maladie."), /* @__PURE__ */ React.createElement("li", null, /* @__PURE__ */ React.createElement("strong", null, "Bouclier"), " (APOTROPA\xCFON) : contre le \xAB mauvais \u0153il \xBB.")) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("li", null, /* @__PURE__ */ React.createElement("strong", null, "Oracle"), " (MANTIS) : r\xE9v\xE9ler l\u2019invisible (pi\xE8ges intermittents)."), /* @__PURE__ */ React.createElement("li", null, "Revoir : Purify (C) et Ward (W) si besoin.")), eduLog.current.slice(-4).map((t, i) => /* @__PURE__ */ React.createElement("li", {
      key: i,
      style: { opacity: 0.85 }
      ...
      style: { fontWeight: 600, marginBottom: 8 }
    }, "Questions flash"), /* @__PURE__ */ React.createElement("ol", {
      style: { paddingLeft: 18, margin: 0 }
    }, level === 1 ? /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("li", null, "Quel os est l\u2019\xAB astragale \xBB ?"), /* @__PURE__ */ React.createElement("li", null, "Pourquoi percer un osselet ?"), /* @__PURE__ */ React.createElement("li", null, "Contre quoi prot\xE8ge le bouclier ?")) : /* @__PURE__ */ React.createElement(React.Fragment, null, /* @__PURE__ */ React.createElement("li", null, "\xC0 quoi sert l\u2019Oracle en jeu ?"), /* @__PURE__ */ React.createElement("li", null, "Comment rep\xE9rer un pi\xE8ge intermittent sans Oracle ?"))))), /* @__PURE__ */ React.createElement("div", {
      style: { display: "flex", gap: 8, justifyContent: "end", marginTop: 12 }
    }, /* @__PURE__ */ React.createElement("button", {
      onClick: () => {
        setSummaryOpen(false);
        softResetForLevel();
      ...
      style: btnDark()
    }, "Recommencer le niveau"), level === 1 ? /* @__PURE__ */ React.createElement("button", {
      onClick: () => {
        setSummaryOpen(false);
        setLevel(2);
      },
      style: primaryBtn(false)
    }, "Niveau suivant \u2192") : /* @__PURE__ */ React.createElement("button", {
      onClick: () => {
        setSummaryOpen(false);
        setLevel(1);
      },
      style: primaryBtn(false)
    }, "Retour niveau 1")))), mobileMode && !inIntro && !summaryOpen && !learnModal && /* @__PURE__ */ React.createElement(React.Fragment, null, !oneButton && /* @__PURE__ */ React.createElement("div", {
      style: { position: "absolute", left: 12, right: 12, bottom: 12, display: "flex", gap: 12, justifyContent: "space-between", pointerEvents: "none" }
    }, /* @__PURE__ */ React.createElement(TouchBtn, {
      label: "\u2190",
    ...
    })), oneButton && /* @__PURE__ */ React.createElement("div", {
      ...
      ref: musicEl,
      preload: "auto",
      src: AUDIO_BASE + AUDIO.music
    }), /* @__PURE__ */ React.createElement("audio", {
      ref: sfxJumpEl,
      preload: "auto",
      src: AUDIO_BASE + AUDIO.jump
    }), /* @__PURE__ */ React.createElement("audio", {
      ref: sfxCatchEl,
      preload: "auto",
      src: AUDIO_BASE + AUDIO.catch
    }), /* @__PURE__ */ React.createElement("audio", {
      ref: sfxOuchEl,
      preload: "auto",
      src: AUDIO_BASE + AUDIO.ouch
    })), /* @__PURE__ */ React.createElement("div", {
      style: { fontSize: 12, color: "#6b7280", marginTop: 8 }
    }, "H\xE9ros via ", /* @__PURE__ */ React.createElement("code", null, "hero.anim.json"), ". Ours via ", /* @__PURE__ */ React.createElement("code", null, "bear.anim.json"), " ou fichiers ", /* @__PURE__ */ React.createElement("code", null, "bear(1..6).png"), "/", /* @__PURE__ */ React.createElement("code", null, "bear (1..6).png"), ". Osselets 2D (mini-jeu) : ", /* @__PURE__ */ React.createElement("code", null, "/assets/games/osselets/audio/img/osselets/osselet (1..5).png"), " (+ ", /* @__PURE__ */ React.createElement("code", null, "osselet-shadow.png"), " optionnel)."))));
