// Modifié à partir du fichier original fourni (osselets-runner.tsx). :contentReference[oaicite:0]{index=0}
 ...
 *  - Bouton **Start** pour démarrer (au lieu d’Espace/Entrée).
 *  - Musique de fond **game-music-1.mp3** en boucle (M pour mute).
 *  - Effets SFX : **jump-sound.mp3**, **catch-sound.mp3**, **ouch-sound.mp3** (joués aux bons moments).
 *
...
// -----------------------------------------------------------------------------
// Réglages AUDIO – mets tes fichiers au bon endroit dans ton hébergement.
// -----------------------------------------------------------------------------
const AUDIO_PATHS = {
  music: "game-music-1.mp3",
  jump: "jump-sound.mp3",
  onCatch: "catch-sound.mp3",
  ouch: "ouch-sound.mp3",
};
...
function AstragalusRunner() {
  ...
  // Aide : lance la musique (autoplay policy → doit être déclenché par un clic)
  function startMusicIfWanted() {
    const el = musicEl.current;
    if (!el) return;
    el.loop = true;
    el.volume = 0.35; // volume modéré
    el.muted = !musicOn;
    if (musicOn) {
      el.currentTime = 0;
      el.play().catch(() => {
        // silencieux si le navigateur bloque ; un autre clic (toggleMusic) relancera.
      });
    }
  }
  function toggleMusic() {
    ...
    if (el) {
      el.muted = !newState;
      if (newState && el.paused) el.play().catch(() => {});
    }
  }
  function playOneShot(ref: React.MutableRefObject<HTMLAudioElement | null>) {
  ...
  const [message, setMessage] = useState<string>(
    "← → bouger | Espace sauter | H cours d’histoire | M musique | Avance à droite ✨"
  );
  ...
  useEffect(() => {
    function down(e: KeyboardEvent) {
      // Empêche le scroll, etc.
      if (["ArrowLeft", "ArrowRight", " ", "Space", "m", "M", "h", "H", "Enter", "p", "P"].includes(e.key))
        e.preventDefault();

      if (inIntro) {
        // On avance l'intro uniquement avec → (plus de Space/Enter pour démarrer)
        if (e.key === "ArrowRight") advanceIntro();
      } else if (!summaryOpen) {
  ...
  }, [inIntro, summaryOpen, musicOn]);

  function advanceIntro() {
    intro.current.step = Math.min(intro.current.step + 1, 5);
    intro.current.t = 0;
    // On ne démarre PLUS automatiquement avec Espace/Enter : c'est le bouton Start qui lance le jeu.
  }
  ...
  function resetStateForLevel(startFromIntro = false) {
    // reset player & world
    player.current.x = 120;
    player.current.y = GROUND_Y - player.current.h;
    player.current.vx = 0;
    player.current.vy = 0;
    player.current.onGround = true;
    player.current.facing = 1;
    player.current.speedMul = 1.0;
    player.current.dirt = 0;
    player.current.runPhase = 0;
    // reset flags
  ...
  function update(dt: number) {
    if (inIntro || summaryOpen) {
      if (inIntro) {
        intro.current.t += dt;
        // auto-advance every ~4s (pour montrer les étapes), mais c'est le bouton Start qui lance.
    ...
    // gravity & jump
    p.vy += 0.8 * dt; // gravité
    if (p.onGround && keys.current["jump"]) {
      p.vy = -14;
      p.onGround = false;
      playOneShot(sfxJumpEl);
    }
    ...
    if (stage.current === "start" && p.x > AMULET1_X - 20) {
      ...
      playOneShot(sfxCatchEl);
      if (historyMode)
        showHistory(
          "Amulettes portées pour la protection et la chance : l’astragale peut être monté en collier."
        );
    ...
    if (stage.current === "postChase" && p.x > AMULET2_X - 20) {
      ...
      if (historyMode)
        showHistory(
          "Motifs de protection/purification : l’amulettique antique combine gestes et objets."
        );
    }
    ...
    if ((stage.current === "purifyAmulet" || stage.current === "postChase") && p.x > AMULET3_X - 20) {
      ...
      if (historyMode)
        showHistory(
          "Apotropaïque (contre le ‘mauvais œil’) : amulettes et symboles pour détourner l’envie/le sort."
        );
    }
    ...
    if ((stage.current === "wardAmulet" || stage.current === "purifyAmulet") && p.x > EVIL_WAVE_START_X) {
      ...
      for (let i = 0; i < n; i++) {
        eyes.current.push({
          x: p.x + 240 + i * 90,
          y: GROUND_Y - 100 - ((i % 3) * 30),
          vx: -(2.2 + ((i % 3) * 0.4) + 0.1 * (levelRef.current - 1)),
          vy: 0,
          alive: true,
        });
      }
      ...
      if (historyMode)
        showHistory(
          "Le ‘mauvais œil’ (baskanía) est conjuré par des amulettes/geste apotropaïques."
        );
    }
    ...
    if (eyes.current.length) {
      ...
      if (eyes.current.length === 0 && stage.current === "evilEyeWave") {
        stage.current = "end";
        setMessage(
          "Bien joué ! Fin de démo — poursuis jusqu’à l’extrémité pour valider le niveau."
        );
      }
    }
  ...
  function renderIntro(ctx: CanvasRenderingContext2D, timeMs: number) {
    ...
    if (step === 0) {
      ...
      ctx.font = "14px ui-sans-serif, system-ui";
      centerText(ctx, "Clic ou → pour avancer. Bouton Start pour jouer.", W / 2, H - 72);
    ...
    if (step === 3) {
      ...
      ctx.fillRect(cx2 - 4, cy2 - 60, 8, 36);
      ctx.beginPath();
      ctx.moveTo(cx2 - 8, cy2 - 24);
      ctx.lineTo(cx2 + 8, cy2 - 24);
      ctx.lineTo(cx2, cy2 - 40);
      ctx.closePath();
      ctx.fill();
      // trou
      ctx.fillStyle = "#7c2d12";
      ctx.beginPath();
      ctx.arc(cx2, cy2 - 6, 2, 0, Math.PI * 2);
      ctx.fill();
      ctx.font = "14px ui-sans-serif, system-ui";
    ...
    if (step === 4) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(cx2 - 80, cy2 - 30);
      ctx.quadraticCurveTo(cx2, cy2 - 60, cx2 + 80, cy2 - 30);
      ctx.stroke();
      drawAstragalusIcon(ctx, cx2, cy2, 26);
      ctx.font = "14px ui-sans-serif, system-ui";
      centerText(ctx, "L’objet devient une amulette. Clique Start pour commencer →", W / 2, 360);
    }

    if (step >= 5) {
      centerText(ctx, "Prêt ? Clique sur le bouton Start pour jouer.", W / 2, H / 2);
    }
  }
  ...
  function drawSheepSilhouette(ctx: CanvasRenderingContext2D, x: number, y: number, w: number, h: number) {
    ...
    ctx.beginPath();
    ctx.moveTo(0, h * 0.4);
    ctx.quadraticCurveTo(w * 0.2, h * 0.1, w * 0.55, h * 0.1);
    ctx.quadraticCurveTo(w * 0.85, h * 0.1, w, h * 0.3);
    ctx.quadraticCurveTo(w * 0.9, h * 0.55, w * 0.6, h * 0.6);
    ctx.quadraticCurveTo(w * 0.35, h * 0.65, w * 0.15, h * 0.6);
    ctx.quadraticCurveTo(0, h * 0.55, 0, h * 0.4);
    ctx.closePath();
    ctx.fill();
    ctx.stroke();
    // pattes
    ctx.fillStyle = "#334155";
    ctx.fillRect(w * 0.2, h * 0.6, 10, h * 0.4);
    ctx.fillRect(w * 0.28, h * 0.6, 10, h * 0.4);
    ctx.fillRect(w * 0.72, h * 0.6, 10, h * 0.4);
    ctx.fillRect(w * 0.8, h * 0.6, 10, h * 0.4);
    ctx.restore();
  }
  ...
  function drawAstragalusIcon(ctx: CanvasRenderingContext2D, x: number, y: number, R: number) {
    ...
    ctx.beginPath();
    ctx.moveTo(-R * 0.7, 0);
    ctx.quadraticCurveTo(0, -R * 0.38, R * 0.7, 0);
    ctx.moveTo(-R * 0.7, 0);
    ctx.quadraticCurveTo(0, R * 0.38, R * 0.7, 0);
    ctx.stroke();
    ctx.restore();
  ...
  function drawHUD(ctx: CanvasRenderingContext2D) {
    ...
    // Music & History indicators (visuels)
    ctx.fillStyle = musicOn ? "#16a34a" : "#ef4444";
    ctx.beginPath();
    ctx.arc(232, 70, 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#0f172a";
    ctx.font = "10px ui-sans-serif, system-ui";
    ctx.fillText("Musique (M)", 246, 74);

    ctx.fillStyle = historyMode ? "#16a34a" : "#ef4444";
    ctx.beginPath();
    ctx.arc(232, 98, 6, 0, Math.PI * 2);
    ctx.fill();
    ctx.fillStyle = "#0f172a";
    ctx.fillText("Cours d’histoire (H)", 246, 102);
    ctx.restore();
  }
  ...
  function drawAmulet(ctx: CanvasRenderingContext2D, x: number, y: number, label: string) {
    ...
    ctx.beginPath();
    ctx.moveTo(-18, -12);
    ctx.quadraticCurveTo(0, -24 - 6 * Math.sin(t), 18, -12);
    ctx.stroke();
  ...
  function drawBear(ctx: CanvasRenderingContext2D, x: number, y: number) {
    ...
    ctx.fillRect(0, 20, 64, 36);
    // head
    ctx.beginPath();
    ctx.arc(54, 26, 14, 0, Math.PI * 2);
    ctx.fill();
    // legs
    ctx.fillRect(6, 56, 10, 12);
    ctx.fillRect(26, 56, 10, 12);
    ctx.fillRect(46, 56, 10, 12);
    // eye
  ...
  function drawHero(
    ...
    ctx.beginPath();
    ctx.moveTo(w / 2 - 8, 22);
    ctx.quadraticCurveTo(w / 2, 18, w / 2 + 8, 22);
    ctx.stroke();
    ...
    if (inv.current.speed || inv.current.purify || inv.current.ward) {
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.ellipse(w / 2, 26, 5, 3.5, 0, 0, Math.PI * 2);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(w / 2 - 3.5, 26);
      ctx.quadraticCurveTo(w / 2, 24.5, w / 2 + 3.5, 26);
      ctx.moveTo(w / 2 - 3.5, 26);
      ctx.quadraticCurveTo(w / 2, 27.5, w / 2 + 3.5, 26);
      ctx.stroke();
    }
    ...
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(w / 2, 12, 8, 0.2, Math.PI - 0.2);
    ctx.stroke();
    ...
    <div className="min-h-screen w-full bg-gradient-to-br from-stone-50 to-stone-200 text-stone-900">
      <div className="max-w-5xl mx-auto px-4 py-6">
        <div className="mb-2 flex items-center gap-2 justify-between">
          ...
          <div className="flex gap-2">
            <button
              ...
              className="px-3 py-1.5 rounded-xl bg-white border border-stone-300 shadow-sm"
            >
            ...
            <button
              ...
              disabled={inIntro || summaryOpen}
              className="px-3 py-1.5 rounded-xl bg-stone-900 text-white shadow hover:shadow-md disabled:opacity-50"
            >
        ...
        <p className="text-sm text-stone-600 mb-3">
          {inIntro
            ? "Cinématique d’intro : clique ou → pour avancer — extraction → nettoyage → perçage → collier. Utilise le bouton Start pour lancer le jeu."
            : "← → pour bouger | Espace pour sauter | P pause | H cours d’histoire | M musique. Avance vers la droite pour déclencher l’histoire (ours, purification, mauvais œil)."}
        </p>
        ...
        <div className="bg-white rounded-2xl p-3 shadow-lg border border-stone-200">
          <div className="w-full overflow-hidden rounded-xl border border-stone-100 relative">
            ...
            {/* OVERLAY: START BUTTON pendant l’intro */}
            {inIntro && (
              <div className="absolute inset-0 flex items-end justify-center pb-6 pointer-events-none">
                <button
                  onClick={startGame}
                  className="pointer-events-auto px-6 py-3 rounded-2xl bg-emerald-600 text-white shadow-lg hover:shadow-xl hover:brightness-105 active:translate-y-px"
                >
                  Start
                </button>
              </div>
              ...
              <div className="absolute inset-0 bg-white/95 backdrop-blur-sm flex items-center justify-center p-6">
                <div className="max-w-3xl w-full bg-white border border-stone-200 rounded-2xl shadow-md p-6">
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <span className="text-xs px-2 py-1 rounded-full bg-stone-800 text-white">
                      Mode amulette
                    </span>
                  </div>
                  ...
                  <div className="mt-4 grid md:grid-cols-2 gap-4 text-sm">
                    <div className="bg-stone-50 rounded-xl p-4 border border-stone-200">
                      ...
                      <ul className="list-disc pl-5 space-y-1">
                        <li>
                          <span className="font-semibold">Localisation</span> : astragale ={" "}
                          <em>talus</em>, dans le <span className="underline">tarse postérieur</span>, sous le{" "}
                          <em>tibia</em>, en avant du <em>calcanéus</em>.
                        </li>
                        <li>
                          <span className="font-semibold">Fonction</span> : transmet la flexion/extension du{" "}
                          <em>jarret</em> vers le pied.
                        </li>
                        <li>
                          <span className="font-semibold">Amulette/protection</span> : motifs{" "}
                          <em>chance/vitesse</em>, <em>purification symbolique</em>, <em>apotropaïque</em> (contre le
                          « mauvais œil »).
                        </li>
                        <li>
                          <span className="font-semibold">Fabrication</span> : extraction post-abattage →
                          nettoyage/polissage → perçage → montage en collier.
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="bg-stone-50 rounded-xl p-4 border border-stone-200">
                      ...
                      <ol className="list-decimal pl-5 space-y-1">
                        <li>
                          <span className="font-semibold">Vrai/Faux</span> : l’astragale est un os du tarse.{" "}
                          <span className="text-stone-500">(V/F)</span>
                        </li>
                        <li>
                          <span className="font-semibold">Choix</span> : l’astragale s’articule avec… <em>tibia</em> /
                          crâne / humérus.
                        </li>
                        <li>
                          <span className="font-semibold">Citer</span> un usage amulette/protection vu dans le niveau.
                        </li>
                        <li>
                          <span className="font-semibold">Pourquoi</span> perce-t-on l’osselet avant de le porter ?
                        </li>
                      </ol>
                    </div>
                  </div>
                  ...
                  <div className="mt-5 flex flex-wrap gap-2 justify-end">
                    <button
                      onClick={() => {
                        resetStateForLevel(true);
                        setPaused(false);
                      }}
                      className="px-4 py-2 rounded-xl bg-white border border-stone-300 shadow-sm"
                    >
                      Revoir la cinématique
                    </button>
                    <button
                      onClick={restartLevel}
                      className="px-4 py-2 rounded-xl bg-stone-700 text-white shadow hover:shadow-md"
                    >
                      Recommencer
                    </button>
                    <button
                      onClick={nextLevel}
                      className="px-4 py-2 rounded-xl bg-emerald-600 text-white shadow hover:shadow-md"
                    >
                      Prochain niveau →
                    </button>
                  </div>
            ...
            {/* ÉLÉMENTS AUDIO (invisibles) */}
            <audio ref={musicEl} src={AUDIO_PATHS.music} preload="auto" />
            <audio ref={sfxJumpEl} src={AUDIO_PATHS.jump} preload="auto" />
            <audio ref={sfxCatchEl} src={AUDIO_PATHS.onCatch} preload="auto" />
            <audio ref={sfxOuchEl} src={AUDIO_PATHS.ouch} preload="auto" />
          </div>

          <div className="text-xs text-stone-500 mt-2">
            Les amulettes d’astragale sont attestées dans l’Antiquité (usages protecteurs, symboliques et ludiques). La
            cinématique présente un scénario non violent : extraction artisanale post-abattage, nettoyage, perçage
            discret puis montage en collier.
          </div>
        </div>
      </div>
    </div>
