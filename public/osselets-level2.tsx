...
// - Utilise THREE global (chargé par la page)
// - Masque les lettres si le trou est occlus par la géométrie (raycaster)

;(() => {
  const { useEffect, useRef, useState } = React;
  const T = (window as any).THREE as typeof THREE;         // THREE global
  ...
  function L2(){
    ...
    const camRef      = useRef<T.PerspectiveCamera|null>(null);
    const modelRef    = useRef<T.Object3D|null>(null);
    const anchorsRef  = useRef<T.Object3D[]>([]);
    const rayRef      = useRef<T.Raycaster|null>(null);
    ...
    useEffect(()=>{
      const onResize = ()=>{
        ...
        ctx.setTransform((w*dpr)/W, 0, 0, (h*dpr)/H, 0, 0); // dessiner en repère W×H
        ctxRef.current = ctx;
      };
      onResize();
    ...
    useEffect(()=>{
      ...
      // Loader (ex: three/examples/js/loaders/GLTFLoader.js => THREE.GLTFLoader)
      const loaderCtor = (T as any).GLTFLoader;
      if (typeof loaderCtor !== "function") {
        console.error("[L2] GLTFLoader introuvable sur THREE. Vérifie le <script> examples/js/loaders/GLTFLoader.js.");
        return;
      }
      ...
      loader.load(GLB, (gltf:any)=>{
        ...
        anchorsRef.current = anchors.slice(0,24); // on s’en tient à 24
    ...
    function project(){
      ...
      holes.current = anchors.map((n,i)=>{
        ...
        // occlusion : rayon caméra → trou ; si hit avant le trou => caché
        let hidden=false;
