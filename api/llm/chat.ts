...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    // Clé utilisateur (facultative) ou clé serveur
    ...
    // Certains modèles (ex: gpt-5 chez toi) imposent temp=1 → on n’envoie pas de temperature
    const modelsWithFixedTemp = new Set<string>(['gpt-5']);
    const payload: Record<string, any> = { model, messages };
    if (typeof temperature === 'number' && !modelsWithFixedTemp.has(model)) {
