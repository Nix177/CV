...
export function buildMessages({ message, liberty, concise, lang }) {
  ...
  const contactInstructions = {
    fr: "Aucun formulaire de contact n'existe sur le site. Si un visiteur souhaite contacter Nicolas, indique que ses coordonnées publiques figurent sur la page CV (/cv) et donne son adresse e-mail publique seulement si elle apparaît dans les éléments récupérés. N'invente jamais de formulaire, de lien ou de coordonnée.",
    en: "There is no contact form on the website. If a visitor wants to contact Nicolas, explain that his public contact details are on the CV page (/cv-en) and provide his public email address only when it appears in the retrieved evidence. Never invent a form, link, or contact detail.",
    de: "Auf der Website gibt es kein Kontaktformular. Wenn jemand Nicolas kontaktieren möchte, erkläre, dass seine öffentlichen Kontaktdaten auf der Lebenslauf-Seite (/cv-de) stehen, und nenne seine öffentliche E-Mail-Adresse nur, wenn sie in den abgerufenen Belegen enthalten ist. Erfinde niemals ein Formular, einen Link oder Kontaktdaten."
  };
  ...
  const instructions = `${localizedInstructions[lang] || localizedInstructions.fr} ${contactInstructions[lang] || contactInstructions.fr}`;
